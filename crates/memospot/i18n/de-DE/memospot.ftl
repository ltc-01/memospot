...
dialog-update-manually-prompt = Fehler beim automatischen Update auf Version { $version }:

    { $error }

    Möchten Sie manuell aktualisieren?
dialog-update-message = Version { $version } steht zum Download zur Verfügung.

    Möchten Sie sie herunterladen?
dialog-update-no-update = Kein Update verfügbar.
dialog-update-title = Update verfügbar
error-config-write-error = Fehler beim Schreiben der Konfigurationsdatei:

    { $error }
error-failed-to-install-webview = WebView konnte nicht installiert werden:

    { $error }

    Bitte manuell installieren.
error-invalid-server-url = Ungültige Remote-Server-URL:
    { $url }
...
panic-backup-directory-is-a-file = Sicherungsverzeichnis existiert als Datei!
    { $dir }
panic-backup-directory-is-not-writable = Backup-Verzeichnis ist nicht beschreibbar!
    { $dir }
panic-config-error = Bitte korrigieren Sie die Konfigurationsdatei
    manuell und starten Sie die Anwendung neu.
panic-config-is-not-a-file = Der angegebene Konfigurationspfad ist ein Verzeichnis.
    Es muss eine Datei sein!
    { $path }
panic-config-is-not-writable = Konfigurationsdatei ist nicht beschreibbar!
    { $file }
...
panic-data-directory-is-not-writable = Datenverzeichnis ist nicht beschreibbar!
    { $dir }
panic-database-file-is-not-writable = Datenbankdatei ist nicht beschreibbar!
    { $file }
...
panic-failed-to-create-data-directory = Datenverzeichnis konnte nicht erstellt werden!
    { $dir }
panic-failed-to-run-database-migrations = Fehler beim Ausführen von Datenbankmigrationen:
    { $error }
panic-failed-to-spawn-memos = Fehler beim Spawnen von Memos
panic-log-config-reset-error = Fehler beim Zurücksetzen der Protokollkonfigurationsdatei:
    { $file }
    Bitte löschen und die Anwendung neu starten.
panic-log-config-write-error = Fehler beim Schreiben der Protokollkonfigurationsdatei:
    { $file }
panic-portpicker-error = Konnte keinen freien Port für Memos finden!
panic-unable-to-create-backup-directory = Sicherungsverzeichnis kann nicht erstellt werden!
    { $dir }
panic-unable-to-find-memos-binary = Memos Server binär nicht gefunden!
panic-unable-to-resolve-custom-data-directory = Fehler beim Auflösen des benutzerdefinierten Memos-Datenverzeichniss!
    { $dir }
...
prompt-config-error-message = Fehler beim Parsen der Konfigurationsdatei:
...
prompt-install-webview-message = Eine WebView ist *erforderlich*, damit diese Anwendung funktioniert,
...
viewmenu-reset-zoom = &Zoom zurücksetzen
viewmenu-toggle-menu-bar = &Menüleiste umschalten
viewmenu-zoom-in = Zoom &rein
viewmenu-zoom-out = Zoom &raus
warn-failed-to-backup-database = Sicherung der Datenbank fehlgeschlagen:
